...
async function createCheckoutSession(req, res) {
  if (!stripe) return fail(res, "Stripe is not configured.", 503);
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { userId: req.user.id, addressId },
  ...
  ok(
    res,
    { sessionId: session.id, url: session.url, orderItems },
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (payment && !payment.orderId) {
      ...
      if (items.length) {
        ...
        if (reserve.ok) {
          const or = await postJson(ORDER(), "/internal/orders/paid", {
          ...
          if (or.ok) {
            payment.orderId = or.data.data.order._id;
            await payment.save();
            await postJson(
              CART(),
              `/internal/cart/${payment.userId}/clear`,
              {},
            );
          }
        }
