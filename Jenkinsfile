pipeline {
    agent {
        kubernetes {
            inheritFrom 'shopsphere-kaniko-agent'
            defaultContainer 'jnlp'
        }
    }

    environment {
        IMAGE_PREFIX = "docker.io/sushritha03/shopsphere"
        TAG = "${BUILD_NUMBER}"
    }
...
stage('Install Dependencies') {
                ...
                container('kaniko') {
                    sh '''
                        ...
                        /kaniko/executor \
                          ...
                          --dockerfile="${WORKSPACE}/microservices/services/auth/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-auth:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/user/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-user:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/product/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-product:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/cart/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-cart:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/order/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-order:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/payment/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-payment:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/address/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-address:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/review/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-review:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/rag/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-rag:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/support/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-support:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/vendor/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-vendor:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/admin/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-admin:${TAG}"

                        /kaniko/executor \
                          --context="${WORKSPACE}/microservices" \
                          --dockerfile="${WORKSPACE}/microservices/services/api-gateway/Dockerfile" \
                          --destination="${IMAGE_PREFIX}-api-gateway:${TAG}"
            ...
            steps {
                container('kubectl') {
                    sh '''
                        ...
                        kubectl -n shopsphere set image deployment/auth-service \
                          auth-service=${IMAGE_PREFIX}-auth:${TAG}

                        kubectl -n shopsphere set image deployment/user-service \
                          user-service=${IMAGE_PREFIX}-user:${TAG}

                        kubectl -n shopsphere set image deployment/product-service \
                          product-service=${IMAGE_PREFIX}-product:${TAG}

                        kubectl -n shopsphere set image deployment/cart-service \
                          cart-service=${IMAGE_PREFIX}-cart:${TAG}

                        kubectl -n shopsphere set image deployment/order-service \
                          order-service=${IMAGE_PREFIX}-order:${TAG}

                        kubectl -n shopsphere set image deployment/payment-service \
                          payment-service=${IMAGE_PREFIX}-payment:${TAG}

                        kubectl -n shopsphere set image deployment/address-service \
                          address-service=${IMAGE_PREFIX}-address:${TAG}

                        kubectl -n shopsphere set image deployment/review-service \
                          review-service=${IMAGE_PREFIX}-review:${TAG}

                        kubectl -n shopsphere set image deployment/rag-service \
                          rag-service=${IMAGE_PREFIX}-rag:${TAG}

                        kubectl -n shopsphere set image deployment/support-service \
                          support-service=${IMAGE_PREFIX}-support:${TAG}

                        kubectl -n shopsphere set image deployment/vendor-service \
                          vendor-service=${IMAGE_PREFIX}-vendor:${TAG}

                        kubectl -n shopsphere set image deployment/admin-service \
                          admin-service=${IMAGE_PREFIX}-admin:${TAG}

                        kubectl -n shopsphere set image deployment/api-gateway \
                          api-gateway=${IMAGE_PREFIX}-api-gateway:${TAG}
                    '''
                }
            }
        }
    ...
    post {
        success {
            echo 'ShopSphere CI/CD completed successfully.'
        }

        failure {
            echo 'ShopSphere CI/CD failed.'
        }
    }
}
